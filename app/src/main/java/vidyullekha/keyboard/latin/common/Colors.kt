...
package vidyullekha.keyboard.latin.common
...
import vidyullekha.keyboard.keyboard.KeyboardTheme.Companion.STYLE_HOLO
import vidyullekha.keyboard.keyboard.KeyboardTheme.Companion.STYLE_MATERIAL
import vidyullekha.keyboard.latin.common.ColorType.*
import vidyullekha.keyboard.latin.R
import vidyullekha.keyboard.latin.utils.adjustLuminosityAndKeepAlpha
import vidyullekha.keyboard.latin.utils.brighten
import vidyullekha.keyboard.latin.utils.brightenOrDarken
import vidyullekha.keyboard.latin.utils.darken
import vidyullekha.keyboard.latin.utils.isBrightColor
import vidyullekha.keyboard.latin.utils.isDarkColor
import vidyullekha.keyboard.latin.utils.isGoodContrast
...
class DynamicColors(context: Context, override val themeStyle: String, override val hasKeyBorders: Boolean, private var keyboardBackground: Drawable? = null) : Colors {
    ...
    private val brahmicKeyText: Int = brahmicLabelColor(keyText, accent, keyBackground)
    ...
    override fun get(color: ColorType): Int = when (color) {
        ...
        BRAHMIC_KEY_TEXT -> brahmicKeyText
...
private fun brahmicLabelColor(keyText: Int, accent: Int, keyBackground: Int): Int {
    val tinted = if (isBrightColor(keyBackground)) {
        if (isBrightColor(accent)) darken(darken(accent)) else accent
    } else {
        if (isDarkColor(accent)) brighten(brighten(accent)) else accent
    }
    return if (isGoodContrast(tinted, keyBackground)) tinted
        else ColorUtils.blendARGB(keyText, tinted, 0.65f)
}
